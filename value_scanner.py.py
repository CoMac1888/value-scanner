...
API_KEY = '8e2403d4149ca74fd3ef3007e4fae874'  # Same key as test_odds.py
...
# Inputs
sport = st.selectbox("Sport:", ['soccer_epl', 'soccer_efl_championship', 'cricket_test_matches', 'rugby_six_nations'], format_func=lambda x: x.replace('soccer_', '').replace('_', ' ').title())
    ...
    if st.session_state.scans < 3:  # Free limit
        with st.spinner("Hunting edges across UK books..."):
            ...
            if response.status_code == 200:
                ...
                st.error(f"API error: {response.status_code} – Check key. Details: {response.text}")
    else:
